...
def chatbot_page():
    ...
    if (
        ...
        and (
            not st.session_state.agent_executor
            or not st.session_state.agent_init_attempted
        )
    ):
        ...
        print(
            "DEBUG main_app: Condition met for agent initialization/re-initialization attempt."
        )
        st.session_state.agent_init_attempted = (
            True  # Mark that we are attempting initialization in this pass
        )
        ...
        if st.session_state.vector_store:
            ...
            print(
                "WARN main_app: Vector store not available during agent initialization attempt. RAG tool may be impaired."
            )
        ...
        if llm_for_agent:
            ...
            tavily_key_for_agent = st.session_state.get(
                "tavily_api_key", TAVILY_API_KEY
            )  # Use session state first, then config

            with st.spinner(
                "Initializing AI Agent... Please wait."
            ):  # Spinner for clarity
                print(
                    f"DEBUG main_app: Calling get_agent_executor with LLM: {st.session_state.selected_model}, Temp: {st.session_state.temperature}"
                )
            ...
            if st.session_state.agent_executor:
                ...
                st.error(
                    "Failed to initialize the AI Agent. Please check configurations or API keys in the sidebar. Some features may be unavailable."
                )
        else:
            print("ERROR main_app: LLM for agent not available. Agent not initialized.")
            st.error(
                "LLM could not be initialized. Agent cannot function. Please check Google API key."
            )
    ...
    with st.sidebar:
        ...
        if st.button("Logout", key="logout_btn_main_app_streaming"):
            ...
            all_keys_to_clear = list(
                st.session_state.keys()
            )  # Get all keys currently in session state
            ...
            for key in all_keys_to_clear:
                if key not in preserved_keys and key not in ["page"]:
                    ...
                    if (
                        key in default_keys
                    ):  # Only delete if it was part of initial setup, or is dynamic
        ...
        # LLM Configuration
        selected_model_val = st.session_state.get(
            "selected_model", GOOGLE_MODELS[0] if GOOGLE_MODELS else "gemini-1.5-flash"
        )
        ...
        selected_model = st.selectbox(
            ...
            GOOGLE_MODELS,
            index=(
                GOOGLE_MODELS.index(selected_model_val)
                if selected_model_val in GOOGLE_MODELS
                else 0
            ),
            ...
            if (
                ...
                with st.sidebar.expander("📊 Knowledge Base Stats", expanded=False):
                    ...
                    if (
                        ...
                        and hasattr(st.session_state.vector_store, "index")
                        and st.session_state.vector_store.index
                    ):
                        kb_stats["vector_count"] = (
                            st.session_state.vector_store.index.ntotal
                        )
                    ...
                    st.write(
                        f"• Vector Store Size: {kb_stats['vector_store_size_mb']} MB"
                    )
                ...
                with st.sidebar.expander("📈 Conversation Metrics", expanded=False):
                    if st.session_state.current_conversation_id:
                        ...
                        if conversation_stats:
                            ...
                            if conv_data["total_messages"] > 0:
                                avg_latency = (
                                    conv_data["total_latency_ms"]
                                    / conv_data["total_messages"]
                                )
                            ...
                            if conv_details and "messages" in conv_details:
                                ...
                                assist_msgs = [
                                    m
                                    for m in conv_details["messages"]
                                    if m["message_type"] == "assistant"
                                ]
                                if assist_msgs:
                                    avg_gen_time = sum(
                                        m["generation_time_ms"] for m in assist_msgs
                                    ) / len(assist_msgs)
                                    avg_ret_time = sum(
                                        m["retrieval_time_ms"] for m in assist_msgs
                                    ) / len(assist_msgs)
                                    avg_docs = sum(
                                        m["documents_retrieved"] for m in assist_msgs
                                    ) / len(assist_msgs)
                    ...
                    # Overall stats
                    aggregated_stats = (
                        st.session_state.conversation_tracker.get_aggregated_stats()
                    )
                    if aggregated_stats:
                        st.write("**Overall Usage:**")
                        st.write(
                            f"• Total Conversations: {aggregated_stats.get('conversation_count', 0)}"
                        )
                        st.write(
                            f"• Total Messages: {aggregated_stats.get('total_messages', 0)}"
                        )
                        st.write(
                            f"• Total Tokens: {aggregated_stats.get('total_tokens', 0)}"
                        )
                ...
                with st.sidebar.expander("📚 Knowledge Base Management", expanded=True):
                        ...
                        if not st.session_state.kb_synced:
                            if st.button("🔄 Full KB Sync", key="resync_kb_btn"):
                                with st.spinner("Syncing knowledge base..."):
                                    updated_vector_store = resync_knowledge_base(
                                        st.session_state.embeddings_model
                                    )
                    ...
                    if st.session_state.document_removed:
                        st.success(
                            f"Document '{st.session_state.removed_document_name}' was removed successfully!"
                        )
                    ...
                    # Display document list with options if no document was just removed
                        ...
                        for doc in st.session_state.kb_documents:
                            ...
                            with col2:
                                if st.button(
                                    "🗑️",
                                    key=f"remove_{doc}",
                                    help=f"Remove {doc} from knowledge base",
                                ):
                        ...
                        st.info(
                            "No documents in knowledge base. Upload documents below."
                        )
    ...
    for msg_idx, msg_data in enumerate(st.session_state.messages):
        ...
        # Clean up the text by removing trailing backticks
        text_to_display = text_to_display.rstrip('`')
    ...
    if prompt := st.chat_input(
        "Ask me anything about your documents or beyond...", disabled=prompt_disabled
    ):
        ...
        if not st.session_state.agent_executor:
            st.error(
                "Agent is not ready. Please check configurations in the sidebar (e.g., API keys) and ensure the agent has initialized."
            )
            st.stop()
        ...
        with st.chat_message("assistant"):
            try:
                ...
                print(
                    f"DEBUG main_app: Invoking Agent with input: '{prompt}', history_len: {len(chat_history_for_agent)}"
                )
                ...
                def stream_agent_response_chunks():
                    if st.session_state.current_conversation_id is None:
                        ...
                        st.session_state.current_conversation_id = (
                            st.session_state.conversation_tracker.start_conversation(
                                user_id
                            )
                        )
                    ...
                    try:
                        for chunk in st.session_state.agent_executor.stream(
                            ...
                            if "actions" in chunk and chunk["actions"]:
                                ...
                                for step in chunk["steps"]:
                                    ...
                                    max_obs_len = 250
                                    truncated_obs = observation_str[:max_obs_len] + (
                                        "..."
                                        if len(observation_str) > max_obs_len
                                        else ""
                                    )
                                    current_thought_process += (
                                        f"**Observation:** `{truncated_obs}`\n\n"
                                    )
                                    message_placeholder.markdown(
                                        current_thought_process
                                    )
                                ...
                                message_placeholder.markdown(
                                    full_agent_output.rstrip('`').replace('\n', '<br>'),
                                    unsafe_allow_html=True,
                                )
                        ...
                        print(
                            f"ERROR main_app: Exception during agent stream processing: {stream_ex}"
                        )
                    ...
                    if full_agent_output:
                        ...
                        full_agent_output = (
                            "*Agent finished processing but did not produce an output.*"
                        )
                        if message_placeholder:
                            message_placeholder.markdown(
                                full_agent_output.rstrip('`').replace('\n', '<br>'),
                    ...
                    st.session_state.messages.append(
                        {
                            "role": "assistant",
                            "content": full_agent_output,
                            "timestamp": datetime.now().timestamp(),
                        }
                    )
                    print(
                        f"DEBUG main_app: Agent full streamed output recorded: {full_agent_output}"
                    )
                ...
                error_message_for_history = "Sorry, an error occurred while the agent was processing your request."
                st.markdown(error_message_for_history)
                st.session_state.messages.append(
                    {
                        "role": "assistant",
                        "content": error_message_for_history,
                        "timestamp": datetime.now().timestamp(),
                    }
                )
...
def admin_page():
    ...
    if aggregated_stats:
        ...
        with col1:
            st.metric(
                "Total Conversations", aggregated_stats.get("conversation_count", 0)
            )
        with col2:
            ...
            if all_convs and len(all_convs) > 1:
                ...
                if time_data:
                    ...
                    token_chart = (
                        alt.Chart(df_time)
                        .mark_line()
                        .encode(x="Date:T", y="Tokens:Q", tooltip=["Date", "Tokens"])
    ...
    if conversations:
            ...
            df_data.append(
                {
                    ...
                    "Avg Latency (ms)": conv.get("total_latency_ms", 0)
                    / max(1, conv.get("total_messages", 1)),
                }
            )
        ...
        if selected_conv_id:
            conv_details = (
                st.session_state.conversation_tracker.get_conversation_details(
                    selected_conv_id
                )
            )
            if conv_details and "messages" in conv_details:
                ...
                for msg in conv_details["messages"]:
                    with st.expander(
                        f"{msg['message_type'].capitalize()} ({msg['timestamp']})",
                        expanded=False,
                    ):
